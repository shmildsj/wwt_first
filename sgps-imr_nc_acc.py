...
def test_classify(feature, labels, args):
    ...
    for train_index, test_index in tqdm.tqdm(kf.split(feature)):
        ...
        pre = precision_score(test_y, preds)
        rec = recall_score(test_y, preds)
        acc, sen, pre, spe, f1, mcc = calculate_metrics(test_y, preds)
        print(acc, sen, pre, spe, f1)
        # 0.8456797887058232 0.036028823058446756 0.6617647058823529 0.9965681886004177 0.06833712984054668
        p, r, f, _ = precision_recall_fscore_support(test_y, preds)
        print(p, r, f)
