...
def evaluate_auprc(train_pred, train_true, val_pred, val_true, test_pred, test_true):
    train_auc = roc_auc_score(train_true, train_pred)
    valid_auc = roc_auc_score(val_true, val_pred)
    test_auc = roc_auc_score(test_true, test_pred)
    train_precision, train_recall, _ = precision_recall_curve(train_true, train_pred)
    train_auprc = auc(train_recall, train_precision)

    valid_precision, valid_recall, _ = precision_recall_curve(val_true, val_pred)
    valid_auprc = auc(valid_recall, valid_precision)

    test_precision, test_recall, _ = precision_recall_curve(test_true, test_pred)
    test_auprc = auc(test_recall, test_precision)
    results = dict()
    results['AUC'] = (train_auc, valid_auc, test_auc)
    results['AUPRC'] = (train_auprc, valid_auprc, test_auprc)
    return results
...
def train(model, predictor, data, split_edge, optimizer, args):
    ...
    edge = pos_train_edge.to(data.x.device)
    pos_out = predictor(h, edge)
    pos_loss = -torch.log(pos_out + 1e-15).mean()

    # pos_edge = split_edge['train']['edge'].t()
    new_edge_index, _ = add_self_loops(data.edge_index.cpu())
    edge = negative_sampling(
        new_edge_index, num_nodes=data.num_nodes,
        num_neg_samples=pos_train_edge.to(data.x.device).shape[1])
...
def test(model, predictor, data, adj, split_edge, batch_size):
    ...
    results = evaluate_auprc(train_pred, train_true, val_pred, val_true, test_pred, test_true)
    return results
...
def main():
    ...
    if args.dataset in ['Cora', 'CiteSeer', 'PubMed']:
        ...
        # data = dataset[0]
         ...
         drug = np.loadtxt("dataset/DrugVirus/drugfeatures.txt")  # MDAD 1373 1720 DrugVirus 175
         microbe = np.loadtxt("dataset/DrugVirus/microbefeatures.txt")  # 173 140 95
         adj = np.loadtxt("dataset/DrugVirus/adj.txt")  # 2470
